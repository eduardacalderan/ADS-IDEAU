...
begin
  ...
  begin
    sql := sql + ' where upper(cidade) like ' + QuotedStr(UpperCase('%' + editNome.Text + '%'));
    condicao := true;
  end;
  ...
  begin
    if not condicao then
      ...
      sql := sql + ' and ';
    sql := sql + ' upper(estado) like ' + QuotedStr(UpperCase('%' + editCPF.Text + '%'));
  end;
  sql := sql + ' order by cidade';
  dtmCadastro2.sqlProcura(sql);
end;
